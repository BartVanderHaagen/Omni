...
void setup()
{
  ...
  nh.advertise(debug_publisher3);
  nh.advertise(debug_publisher4);
  // RT
...
void loop() {
  ...
  if(_frequency_rate.delay(millis())) {
   ...
   // MOTOR BACK RIGHT
    ...
    motor_left_rate_ref  = (linear_velocity_ref - BASE_LENGTH / 2.f * angular_velocity_ref)
               / (WHEEL_RADIUS);
    motor_back_right_rate_ref = (linear_velocity_ref + BASE_LENGTH / 2.f * angular_velocity_ref)
               / (WHEEL_RADIUS);
    motor_back_left_rate_ref  = (linear_velocity_ref - BASE_LENGTH / 2.f * angular_velocity_ref)
               / (WHEEL_RADIUS);
  ...
  if(_frequency_controller.delay(millis())) {
    ...
    // DEBUG
    debug_back_left.x = motor_back_left_rate_ref;
    debug_back_left.y = motor_back_left_rate_est;
    debug_back_left.z = pwmMotorBackLeft;
    debug_publisher3.publish(&debug_back_left);

    // DEBUG
    debug_back_right.x = motor_back_right_rate_ref;
    debug_back_right.y = motor_back_right_rate_est;
    debug_back_right.z = pwmMotorBackRight;
    debug_publisher4.publish(&debug_back_right);
